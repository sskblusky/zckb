...
Page({
  ...
  getInfo: function () {
    ...
    wx.request({
      url: "http://202.43.159.130:6828/index.asp",
      data: {
        sort: that.data.listsort,
  ...
  getList: function (matchids, date, done, err) {
    ...
    if (date == "") {
      url = "http://202.43.159.130:6828/list.asp";
      data = {
        sort: that.data.listsort,
        matchids: matchids
      };
    } else {
      url = "http://202.43.159.130:6828/listback.asp";
      data = {
        date: date
      };
    }
      ...
      success: function (res) {
        if (res.data.length == 0) {
          ...
          if(that.data.searchBack){
            ...
            var list =  that.data.page > 1 ? that.data.list.concat(res.data) : res.data;
  ...
  refesh: function (e) {
    var that = this;
    if (that.data.searchBack) return;
    that.setData({
      page: 1
    });
    that.getInfo();
  ...
  bindCalendarHide: function (e) {
    ...
    if(that.data.searchBack){
      ...
      that.showCover("calendar", false, null);
    }
  },
  bindCalendarTap: function (e) {
    ...
    that.getList(
      "",
      that.data.cur_year + "-" + that.data.cur_month + "-" + e.currentTarget.dataset.day,
      function () {
        that.setData({
          tabClander: that.data.cur_month + "-" + e.currentTarget.dataset.day,
          matchStat: "今日比赛",
          matchCount: that.data.list.length,
          moreClass: "moreHidden",
          coverHidden: true,
          filterHidden: true,
          calendarHidden: true,
          calendarShowHidden: false
        });
      },
      function () {
    ...
    if (that.data.currentTab === navId) {
      ...
      switch (navId) {
        ...
        case "1":
          ...
          break;
        case "2":
          if (that.data.searchBack) break;
          that.showCover("filter", true, navId);
          break;
        case "3":
          that.showCover("calendar", true, navId);
          break;
      }
    }
  },
